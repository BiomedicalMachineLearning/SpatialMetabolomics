...
save_results <- function(tfit, out_dir, species, suffix, save=T) {
  ...
  if (save) {
    file_name <- paste(out_dir, 'de_results_PseudoLimma', '_allGenes_', suffix, '.csv', sep='')
    file_name_up_all <- paste(out_dir, 'de_results_PseudoLimma_up_allGenes_', suffix, '.csv', sep='')
    file_name_down_all <- paste(out_dir, 'de_results_PseudoLimma_down_allGenes_', suffix, '.csv', sep='')
    
    write.csv(up_all_df, file_name_up_all, col.names=TRUE, row.names=TRUE) 
    write.csv(down_all_df, file_name_down_all, col.names=TRUE, row.names=TRUE)
    write.csv(down_all_df, file_name,col.names=TRUE, row.names=TRUE, append=T)
    
    # Saving results for significant genes #
    file_name <- paste(out_dir, 'de_results_PseudoLimma', '_', suffix, '.csv', sep='')
    file_name_up <- paste(out_dir, 'de_results_PseudoLimma_up_', suffix, '.csv', sep='')
    file_name_down <- paste(out_dir, 'de_results_PseudoLimma_down_', suffix, '.csv', sep='')
    
    if (nrow(up_df)!=0) {
      write.csv(up_df, file_name_up, col.names=TRUE, row.names=TRUE)
    }
    
    if (nrow(down_df)!=0) {
      write.csv(down_df, file_name_down, col.names=TRUE, row.names=TRUE)
    }
    
    if (nrow(down_df)!=0) {
        write.csv(down_df, file_name, col.names=TRUE, row.names=TRUE)
    }
  } else {
    print(dim(up_all_df))
