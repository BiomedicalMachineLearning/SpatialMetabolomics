...
save_results <- function(tfit, out_dir, species, suffix, save=TRUE) {
  ...
  if (save) {
    ...
    writeData(wb, sheet = sheet_up, up_all_df,colNames=TRUE, rowNames=TRUE)
    writeData(wb, sheet = sheet_down, down_all_df,colNames=TRUE, rowNames=TRUE)
    
    saveWorkbook(wb, file = file_name, overwrite = TRUE)
    
    # Saving results for significant genes #
    file_name_sign <- paste(out_dir, 'de_results_PseudoLimma_', suffix, '.xlsx', sep='')
    sheet_up1 <- paste(species[1], '.treatment_up', sep='')
    sheet_down1 <- paste(species[1], '.treatment_down', sep='')
    
    wb_sign <- createWorkbook()
    
    addWorksheet(wb_sign, sheet_up1)
    addWorksheet(wb_sign, sheet_down1)
      
    if (nrow(up_df)!=0) {
      writeData(wb_sign, sheet = sheet_up1, up_df, colNames=TRUE, rowNames=TRUE)
    }
    
    if (nrow(down_df)!=0) {
      writeData(wb_sign, sheet = sheet_down1, down_df, colNames=TRUE, rowNames=TRUE)
    }
    saveWorkbook(wb_sign, file = file_name_sign, overwrite = TRUE)
