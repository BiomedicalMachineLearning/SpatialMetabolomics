...
save_gseaResults <- function(em, result, out_dir, suffix) {
  ...
  write.xlsx(result, file_name,
             sheetName=sheet_name, colNames=TRUE, rowNames=TRUE, append=F)
}

format_go_names <- function(go_names) {
  go_names <- str_replace_all(go_names, 'HALLMARK_DNA_REPAIR', 'DNA_REPAIR_hmk')
  ...
  go_names <- str_replace_all(go_names, "GOBP_REGULATION_OF_PROTEIN_SERINE_THREONINE_KINASE_ACTIVITY", 
