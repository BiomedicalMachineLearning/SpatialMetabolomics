#' @DataBaseSearchingPipeline_mod_for_TS
#' Authored by: Christopher CJ Fitzgerald, July 2023.
#' For use of this pipeline in publications please ask Christopher Fitzgerald, Bioinformatician @MetabolomicsAustralia.
#' github https://github.com/ChemCharles
#'
#' @description
#' The below code is a test pipeline used initially to develop the functions in the db explorer application. This pipeline allows the user to search a list of m/z and RT values derived from HRAM experiments against a custom database of mz and rt.
#'
#' This was modified by Christopher Fitzgerald for Thomas Soerianto
#'
#' The pipeline is made up of a single function
#'#' @proc_db
#' This is the main function. It does a few things. Probably best if you just read the comments below.
#' @observed_df The dataframe of experimental data. To be searched against, i.e., row_id and m/z and rt.min, There is a very specific format to this database please see the input test files located @inst/custom_db_search/Query_list.csv
#' @reference_df A dataframe of a database to be aligned. There is a very specific format to this database please see the input test files located @inst/custom_db_search/Query_Custom.csv.
#' @ppm_threshold The ppm threshold that is to be used to search through the database.
#'@RT_tol The retention time tolerance in minutes. defualt at 0.25 minutes

# load libraries required for the functions.
library(dplyr)
library(data.table)
library(tidyr)

# find the test files in inst/custom_db_search

# run the function to search the databases.
test_df <- proc_db_custom(Query_list,Query_Custom_DB,ppm_threshold = 10, RT_tol = 0.3)

proc_db_custom <- function(observed_df,
                    reference_df,
                    ppm_threshold = 10,
                    RT_tol = 0.25) {
  ...
  # for (col_name in names(reference_df)[-c(1:5)]) {
    ...
    for (i in seq_len(nrow(observed_df))) {
      ...
      filtered_matches <- data.frame(
        ...
        Reference_mz = reference_df[[col_name]][matching_indices],
        Error = error,
        RT_obsvered = observed_df$rt.min[i],
        RT_DB = reference_df$rt.min[matching_indices],
        RT_diff = observed_df$rt.min[i] - reference_df$rt.min[matching_indices],
        RT_within_range = abs(observed_df$rt.min[i] - reference_df$rt.min[matching_indices]) <= RT_tol,
        DB_ID = reference_df$row_id[matching_indices],
        Formula = reference_df$Formula[matching_indices],
        HMDBID = reference_df$HMDBID[matching_indices],
        KEGGID = reference_df$KEGGID[matching_indices]
